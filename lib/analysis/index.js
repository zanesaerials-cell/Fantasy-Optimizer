...
/** Below this, an "upgrade" is noise — a rounding difference, not a decision. */
export const MEANINGFUL_UPGRADE = 1.5;
...
export function rankWaiverTargets({ candidates, myRoster, slots, preference = "balanced" }) {
  ...
  return candidates
    .map((c) => {
      ...
      if (c.projection?.confidence?.level === "LOW" || c.projection?.confidence?.level === "UNAVAILABLE")
        reasonsAgainst.push("Low projection confidence");
      if (slotsAvailable === 0)
        reasonsAgainst.push(`No ${c.position} slot in your lineup`);
      if (upgrade < MEANINGFUL_UPGRADE)
      ...
      return {
        ...
        bestUse,
        reasonsAgainst,
        // Meaningful = actually worth a roster move, not just "exists".
        meaningful: upgrade >= MEANINGFUL_UPGRADE && slotsAvailable > 0,
...
/**
 * Waiver tab buckets. "Best" and "immediate" only show players actually
 * worth a roster move — a page full of marginal free agents isn't useful,
 * it's noise. Upside/floor stay broader since a person browsing those
 * tabs is explicitly looking past the immediate-upgrade bar.
 */
export function waiverBuckets(ranked) {
  const meaningful = ranked.filter((p) => p.meaningful);
  return {
    best: meaningful.slice(0, 25),
    immediate: meaningful.filter((p) => p.bestUse.startsWith("Immediate")).slice(0, 25),
    upside: ranked
      .filter((p) => p.projection?.ceiling != null && p.upgrade > 0)
      .sort((a, b) => (b.projection.ceiling || 0) - (a.projection.ceiling || 0))
      .slice(0, 25),
    floor: ranked
      .filter((p) => p.projection?.floor != null && p.upgrade > 0)
