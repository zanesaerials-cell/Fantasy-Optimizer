...
export function project(player, history = [], marketPts = null) {
  ...
  // Sleeper we compute it ourselves from raw weekly history. Only the
  // locally-computed path needs the games-played warning label — ESPN's
  // number is trustworthy regardless of how much history we happen to have
  // fetched this request.
  const platformSeasonAvg = player.seasonAvg ?? null;
  const computedSeasonAvg = played.length >= 2 ? mean(played) : null;

  const candidates = {
    [SOURCE.MARKET]: marketPts,
    [SOURCE.PLATFORM]: player.weekProjected ?? null,
    [SOURCE.RECENT]: recent.length >= 2 ? mean(recent) : null,
    // A single played week is one data point, not an average — a player
    // who was inactive for one game and logged a 0 would otherwise show up
    // as "projected 0.0" with no indication that's built on n=1.
